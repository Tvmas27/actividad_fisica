...
// Datos almacenados globalmente para filtros
let globalData = {
    rango: null,
    sedentarismo: null,
    tipoAct: null,
...
let filtroActivos = 25; // % mínimo de actividad para filtrar
...
function actualizarGrafico(rangoData, sedData, filtroMinutos) {
  const ctx = document.getElementById('chartRangos');
  if (!ctx) return;
  const labels = ['14-24','25-40','41-60','60+'];
  const minutos = [0,0,0,0], sed = [0,0,0,0];

  // Aplicar filtro: solo mostrar rangos con minutos >= filtroMinutos
  const filtro = filtroMinutos || 0;
  rangoData?.forEach(item => {
    const i = labels.indexOf(item.rango);
    if (i>=0 && item.promedio_minutos >= filtro) {
      minutos[i] = Math.round(item.promedio_minutos);
    }
  });
  sedData?.forEach(item => {
    const i = labels.indexOf(item.rango);
    if (i>=0 && minutos[i] > 0) { // solo mostrar si el rango pasó el filtro
      sed[i] = Math.round(item.porcentaje_sedentario);
    }
  });
...
// Filtrar tabla de rangos según el porcentaje de actividad
function filtrarTablaRangos(filtroMinutos) {
  const tbodyC = document.getElementById('tabla-rangos');
  if (!tbodyC || !globalData.rango) return;
  ...
  orden.forEach(r => {
    ...
    if (rItem && rItem.promedio_minutos >= filtroMinutos) {
  ...
  tbodyC.innerHTML = html || '<tr><td colspan="4">No hay datos que cumplan el filtro</td></tr>';
}
...
async function cargarDashboard() {
  ...
  // KPI 2 y 3 (aplicar filtro inicial)
  if (rango && sedentarismo) {
    const avgMin = rango.reduce((s,r) => s + parseFloat(r.promedio_minutos||0), 0) / rango.length;
    const avgSed = sedentarismo.reduce((s,r) => s + parseFloat(r.porcentaje_sedentario||0), 0) / sedentarismo.length;
    setText('kpi-minutos-rango', Math.round(avgMin) + ' min promedio');
    setText('hero-minutos', Math.round(avgMin) + ' min');
    setText('kpi-sedentarismo-rango', Math.round(avgSed) + '% promedio');
  ...
  // KPI 6
  if (usaApp) {
    const si = usaApp.find(a => a.usa_app === 'Sí') || { porcentaje: 0 };
  ...
  sliders.forEach(s => {
    ...
    slider.addEventListener('input', function() {
      ...
      if (s.tipo === 'activos') {
        ...
        // FILTRO REAL: Actualizar gráfico y tabla con el nuevo umbral
        filtroActivos = val;
